...
exports.designButtons = designButtons;
...
function setCanvasSize(width, height) {
  d3.select('.main-svg').attr('width', width).attr('height', height);
}
/**
...
function drawButtons(width) {
  var g = d3.select('.main-svg');
  var button1 = g.append('g').attr('class', 'button').attr('id', 'button1').attr('transform', 'translate(' + width / 6 + ', 50)').attr('width', 130).attr('height', 25);
  var button2 = g.append('g').attr('class', 'button').attr('id', 'button2').attr('transform', 'translate(' + width / 2 + ', 50)').attr('width', 130).attr('height', 25);
  var button3 = g.append('g').attr('class', 'button').attr('id', 'button3').attr('transform', 'translate(' + 5 * width / 6 + ', 50)').attr('width', 130).attr('height', 25);
  var buttons = d3.selectAll('.button');
  buttons.append('rect').attr('width', 130).attr('height', 30).attr('fill', '#f4f6f4').on('mouseenter', function () {
...
function designButtons(button, text) {
  button.append('text').attr('x', 65).attr('y', 15).attr('text-anchor', 'middle').attr('dominant-baseline', 'middle').attr('class', 'button-text').text(text).attr('font-size', '10px').attr('fill', '#362023');
}
/**
...
function addFooter(g) {
  g.append('text').text('Lien vers le site des données:').attr('class', 'footer').attr('transform', 'translate(100, 540)');
  g.append('text').text('https://www.donneesquebec.ca/recherche/dataset/perspective-d-emploi-par-industrie-2021-2025').attr('class', 'footer').attr('transform', 'translate(100, 555)').attr('fill', 'blue');
  g.append('text').text('https://www.donneesquebec.ca/recherche/dataset/perspective-d-emploi-par-profession-2021-2025').attr('class', 'footer').attr('transform', 'translate(100, 570)').attr('fill', 'blue');
}
},{}],"scripts/viz1/preprocess1.js":[function(require,module,exports) {
"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.getIndustryNames = getIndustryNames;
exports.getDataByIndustry = getDataByIndustry;
exports.adjustIndustryNames = adjustIndustryNames;
exports.adjustRegions2021 = adjustRegions2021;

/**
 * @param {Array} data data to extract unique industry names
 * @returns {Array} unique industry names
 */
function getIndustryNames(data) {
  var uniqueNames = [];
...
function getDataByIndustry(data, industrySelected) {
  var dataByIndustry = [];
...
function adjustIndustryNames(_data) {
  var newData = _data.map(function (row) {
...
function adjustRegions2021(data2021) {
  var correctedNames = {
...
},{}],"scripts/viz1/colorbar.js":[function(require,module,exports) {
"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.createColorGradient = createColorGradient;
exports.drawColorBar = drawColorBar;
exports.colorBarRangeText = colorBarRangeText;

var viz1 = _interopRequireWildcard(require("./viz1.js"));
 ...
 */
function createColorGradient(graph) {
  var gradient = graph.append('linearGradient').attr('id', 'svgGradient').attr('x1', '100%').attr('x2', '100%').attr('y1', '0%').attr('y2', '100%');
  gradient.append('stop').attr('class', 'start').attr('offset', '0%').attr('stop-color', viz1.colorMax).attr('stop-opacity', 1);
  gradient.append('stop').attr('class', 'end').attr('offset', '100%').attr('stop-color', viz1.colorMin).attr('stop-opacity', 1);
}
/**
 * @param {*} graph SVG graph object
...
function drawColorBar(graph, _graphSize) {
  createColorGradient(graph);
  graph.append('rect').attr('x', _graphSize.width / 2).attr('y', 0).attr('height', _graphSize.height + 90).attr('width', 25).attr('fill', 'url(#svgGradient)');
}
/**
...
function colorBarRangeText(minValue, maxValue, graph) {
  graph.selectAll('#legend-range').remove();
  graph.append('text').attr('x', 320).attr('y', 10).attr('id', 'legend-range').text(maxValue);
  graph.append('text').attr('x', 330).attr('y', 450).attr('id', 'legend-range').text(minValue);
}
},{"./viz1.js":"scripts/viz1/viz1.js"}],"scripts/viz1/map.js":[function(require,module,exports) {
"use strict";
...
exports.default = color2018;
exports.color2020 = color2020;
exports.mapColoring = mapColoring;
exports.toolTipVisible = toolTipVisible;
exports.getHtmlTooltip = getHtmlTooltip;
exports.drawMap = drawMap;

var colorbar = _interopRequireWildcard(require("./colorbar.js"));

var preprocess = _interopRequireWildcard(require("./preprocess1.js"));
 ...
 */
function mapBackground(data, path, graph) {
  graph.append('path').datum(data).attr('d', path).attr('stroke', '#000000').attr('stroke-width', '1').attr('class', 'region').attr('id', data.properties.res_nm_reg).style('fill', 'none');
}
/**
...
function color2018(industrySelected, graph, data2019, regions, regionName) {
  var dataByIndustry = preprocess.getDataByIndustry(data2019, industrySelected);
  dataByIndustry = dataByIndustry.filter(function (el) {
    return regions.features.some(function (f) {
      return f.properties.res_nm_reg === el.nom_region;
    });
  });
  var dataByRegion = dataByIndustry.filter(function (el) {
    return el.nom_region === regionName;
  });
  var valsEmploy = dataByIndustry.map(function (a) {
    return parseInt(a.emploi_arrondi_2018);
  });
  ...
  if (valsEmploy.length > 0) {
    ...
    valsEmploy = valsEmploy.map(function (value) {
      return isNaN(value) ? 0 : value;
    });
    minValue = 0;
    maxValue = Math.max.apply(Math, _toConsumableArray(valsEmploy));
  } else {
    colorValue = 0;
    minValue = 0;
    maxValue = 1;
  ...
  var mycolor = d3.scaleLinear().domain([minValue, maxValue]).range([viz1.colorMin, viz1.colorMax]);
...
function color2020(industrySelected, graph, data2021, regions, regionName) {
  ...
  var valsEmploy = dataByIndustry.map(function (a) {
    return parseInt(a.emploi_arrondi_2020);
  });
  var minValue, maxValue;
  var colorValue;
...
function mapColoring(industrySelected, yearSelected, element, graph, data2019, data2021, regions) {
  ...
  if (parseInt(yearSelected) === 2018) {
    graph.select('#' + regionName).style('fill', returnValue2018[1](returnValue2018[0]));
  } else {
    graph.select('#' + regionName).style('fill', returnValue2020[1](returnValue2020[0]));
  }
}
/**
 * @param {*} industrySelected Industry selected on the dropdown box
...
function toolTipVisible(industrySelected, element, graph, data2019, data2021, regions, path) {
  var regionName = element.properties.res_nm_reg;
  var returnValue2018 = color2018(industrySelected, graph, data2019, regions, regionName);
  var returnValue2020 = color2020(industrySelected, graph, data2021, regions, regionName);
  var centroid = path.centroid(element.geometry);
  var x = centroid[0];
  var y = centroid[1]; //   var text = getHtmlTooltip(returnValue2018, returnValue2020, regionName)

  graph.select('#' + regionName).on('mouseover', function (d) {
    var tip = graph.append('rect').attr('id', 'tooltip').attr('width', 300).attr('height', 110).attr('x', x).attr('y', y - 70);
    tip.style('opacity', 0.6).attr('color', '#000000');
...
function getHtmlTooltip(graph, returnValue2018, returnValue2020, regionName, x, y) {
  var title = 'Information supplémentaire:';
  graph.append('text').attr('id', 'tooltip-text').text(title).attr('fill', 'gold').attr('x', x).attr('y', y - 50);
  var region = 'Nom de Region: ' + regionName;
  graph.append('text').attr('id', 'tooltip-text').text(region).attr('fill', '#ffffff').attr('x', x).attr('y', y - 25);
  var nombre1 = 'Emploi arrondi 2018: ' + returnValue2018[0];
  graph.append('text').attr('id', 'tooltip-text').text(nombre1).attr('fill', '#ffffff').attr('x', x).attr('y', y);
  var nombre2 = 'Emploi arrondi 2018: ' + returnValue2020[0];
  graph.append('text').attr('id', 'tooltip-text').text(nombre2).attr('fill', '#ffffff').attr('x', x).attr('y', y + 25);
}
/**
...
function drawMap(g, industrySelected, yearSelected, _graphSize, map, data2019, data2021) {
...
},{"./colorbar.js":"scripts/viz1/colorbar.js","./preprocess1.js":"scripts/viz1/preprocess1.js","./viz1.js":"scripts/viz1/viz1.js"}],"scripts/viz1/selectionboxes.js":[function(require,module,exports) {
"use strict";
...
exports.createYearSelectBox = createYearSelectBox;
exports.createIndustrySelectBox = createIndustrySelectBox;
 ...
 */
function createSelectionBox(graph, data2019, data2021, graphSize, mapData) {
  ...
  names = names.filter(function (i, p) {
    return names.indexOf(i) === p;
  });
  var years = [2018, 2020];
...
function createYearSelectBox(graph, graphSize, data2019, data2021, mapData, years) {
  var labelsPosX = 500;
  var labelsPosY = 200;
  graph.append('text').text('Année:').attr('x', labelsPosX).attr('y', labelsPosY - 5);
  var select = graph.append('foreignObject').attr('width', 54).attr('height', 19).attr('x', labelsPosX).attr('y', labelsPosY).append('xhtml:body').append('div').append('select').attr('id', 'dropdown-year').on('change', function (d) {
  ...
  select.selectAll('option').data(years).enter().append('option').text(function (d) {
    return d;
...
function createIndustrySelectBox(graph, graphSize, data2019, data2021, mapData, names) {
  var labelsPosX = 500;
  var labelsPosY = 50;
  graph.append('text').text('Industrie sélectionnée:').attr('x', labelsPosX).attr('y', labelsPosY - 5);
  var select = graph.append('foreignObject').attr('width', 300).attr('height', 19).attr('x', labelsPosX).attr('y', labelsPosY).append('xhtml:body').append('div').append('select').attr('id', 'dropdown-industry').on('change', function (d) {
    var selectedValue = d3.select('#dropdown-industry').property('value');
    var selectedYear = d3.select('#dropdown-year').property('value');
    graph.selectAll('.region').remove();
    map.drawMap(graph, selectedValue, selectedYear, graphSize, mapData, data2019, data2021);
  });
  select.selectAll('option').data(names).enter().append('option').text(function (d) {
    return d;
  }).property('selected', function (d) {
    return d === 'Total';
  });
}
},{"./preprocess1.js":"scripts/viz1/preprocess1.js","./map.js":"scripts/viz1/map.js"}],"scripts/viz1/viz1.js":[function(require,module,exports) {
"use strict";
...
exports.build = build;
exports.colorMax = exports.colorMin = void 0;

var preprocess = _interopRequireWildcard(require("./preprocess1.js"));

var colorbar = _interopRequireWildcard(require("./colorbar.js"));

var selectionboxes = _interopRequireWildcard(require("./selectionboxes.js"));
...
var colorMin = '#ffffff';
exports.colorMin = colorMin;
var colorMax = '#f70d1a';
exports.colorMax = colorMax;
 ...
 */
function build(g, graphSize) {
  g.selectAll('*').remove();
  helper.addFooter(g);
  g.append('text').text("L'offre d'emploi par région et par industrie").attr('x', graphSize.width / 2).attr('y', -75).attr('class', 'vizTitle');
  d3.csv('./par_industrie_2019.csv').then(function (data2019) {
    d3.csv('./par_industrie_2021.csv').then(function (data2021) {
      d3.json('./quebec_regions.geojson').then(function (mapData) {
        var industry = 'Total';
        var year = 2018;
...
},{"./preprocess1.js":"scripts/viz1/preprocess1.js","./colorbar.js":"scripts/viz1/colorbar.js","./selectionboxes.js":"scripts/viz1/selectionboxes.js","./map.js":"scripts/viz1/map.js","../helper.js":"scripts/helper.js"}],"scripts/viz2/preprocess2.js":[function(require,module,exports) {
"use strict";
...
function filterRegion(data) {
  data = data.filter(function (element) {
    return element.nom_region === 'Ensemble du Québec';
  });
  return data;
}
/**
...
function getIndustries(data2019, data2021) {
  var industries = {};
  var industries2019 = {
    year: '2019'
  };
  ...
  var stack = [];
  data2019.forEach(function (element) {
  ...
  data2021.forEach(function (element) {
  ...
  var industries2019_2 = {
    year: '2019'
  };
  var industries2021_2 = {
    year: '2021'
  };
  var industries2023_2 = {
    year: '2023'
  };
  var industries2025_2 = {
    year: '2025'
  };
  var exploitation_de_ressources = ['Agriculture/ pêche et chasse', 'Foresterie et Exploitation forestière', 'Extraction minière', 'Construction', 'Pétrole/ charbon et produits chimiques'];
  var industries_lourdes = ['Produits en plastique et en caoutchouc', 'Produits en bois', 'Produits minéraux non métalliques', 'Première transformation des métaux', 'Produits métalliques', 'Machines', 'Produits informatiques/ électroniques et électriques', 'Produits aérospatiaux et leurs pièces', 'Activités diverses de fabrication', 'Autres matériel de transport', 'Textiles et produits textiles et Vêtements et produits en cuir', 'Papier', 'Meubles et produits connexes'];
  var commerce = ['Commerce de gros', 'Commerce de détail', 'Transport et entreposage', 'Impression et activités connexes de soutien', 'Commerce', 'Fabrication', 'Aliments/ boissons et tabac'];
  var technologie_finance = ['Finances', "Sociétés d'assurance et activités connexes", 'Services immobiliers et services de location et de location à bail', 'Services juridiques et de comptabilité', 'Architecture/ génie et services connexes et services spécialisés de design', 'Conception de systèmes informatiques et services connexes', 'Services de conseils en gestion et de conseils scientifiques et techniques ET Services de recherche et de développement scientifiques', 'Publicité/ relations publiques et services connexes ET Autres services professionnels/ scientifiques et techniques', "Gestion de sociétés et d'entreprises ET Services administratifs/ de soutien et autres"];
  var culture = ["Industrie de l'information et industrie culturelle", 'Arts/ spectacles et loisirs'];
  var services_publiques = ['Soins de santé', 'Assistance sociale', 'Hébergement et restauration', 'Autres services (sauf les administrations publiques)', 'Administrations publiques', "Services d'enseignement", 'Services publics'];
  ...
  Object.keys(industries2019).slice(2).forEach(function (element) {
...
},{}],"scripts/viz2/axis2.js":[function(require,module,exports) {
"use strict";
...
function appendGraphLabels(g) {
  g.append('text').text('Années').attr('class', 'y-axis-text').attr('font-size', 12).attr('transform', 'translate( 280, ' + 420 + ')');
  g.append('text').text('Nombre d\'emplois').attr('class', 'x-axis-text').attr('font-size', 12).attr('transform', 'rotate(-90) translate( -200, -60 )');
}
/**
...
function drawXAxis(xScale, height) {
  d3.select('.x-axis').attr('transform', 'translate( 0, ' + height + ')').call(d3.axisBottom(xScale));
}
/**
...
},{}],"scripts/viz2/scales.js":[function(require,module,exports) {
"use strict";
...
exports.setAdaptedYScale = setAdaptedYScale;
...
function setAdaptedYScale(height, val) {
  var v = String(val).length - 1;
  var dam = Math.ceil(val / Math.pow(10, v)) * Math.pow(10, v);
  return d3.scaleLinear().range([0, height]).domain([dam, 0]);
}
},{}],"scripts/viz2/tooltip.js":[function(require,module,exports) {
"use strict";
...
},{}],"scripts/viz2/legend.js":[function(require,module,exports) {
"use strict";
 ...
 */
function drawLegend(colors, g, width) {
  g.append('text').text('Cliquez sur les barres pour').attr('font-size', 12).attr('transform', 'translate(645 ,200)');
  g.append('text').text('obtenir un affichage détaillé').attr('font-size', 12).attr('transform', 'translate(645,220)');
  var legend = g.selectAll('.legend').data(colors).enter().append('g').attr('class', 'legend').attr('transform', function (d, i) {
    return 'translate(-180,' + i * 19 + ')';
  });
  ...
  legend.append('text').attr('x', width + 5).attr('y', 9).attr('dy', '.35em').style('text-anchor', 'start').text(function (d, i) {
    switch (i) {
      ...
      case 2:
        return 'Exploitation de ressources';

      case 3:
        return 'Technologies et finance';

      case 4:
        return 'Service publique';
...
},{}],"scripts/viz2/bars.js":[function(require,module,exports) {
"use strict";
 ...
 */
function drawBars(g, ind, xScale, yScale, tip, colors, graphSize) {
  var keys = Object.keys(ind[0]).slice(1);
  var stackedData = d3.stack().keys(keys)(ind);
  g.selectAll('g.bars').data(stackedData).enter().append('g').style('fill', function (d, i) {
    return colors[i];
  }).selectAll('rect').data(function (d) {
    return d;
  }).enter().append('rect').attr('x', function (d) {
    return xScale(d.data.year) + 25;
  }).attr('y', function (d) {
    return yScale(d[1]);
  }).attr('height', function (d) {
    return yScale(d[0]) - yScale(d[1]);
  }).attr('width', 100).attr('class', 'rec').on('mouseover', tip.show).on('mouseout', tip.hide).on('click', function (d) {
    tip.hide();
...
function detail(g, d, ind, colors, graphSize, xScale, tip) {
  setClickHandler(g, graphSize);
  var keys = Object.keys(ind[0]);
  var val = d[1] - d[0];
  var idx = Object.values(d.data).indexOf(val);
  var singleStack = [];
  Object.keys(ind).forEach(function (element) {
    var temp = {};
    var name = keys[idx];
  ...
  var k = [keys[idx]];
  var stackedData = d3.stack().keys(k)(singleStack);
  g.selectAll('.rec').remove();
  g.selectAll('.legend').remove();
  var yScale = scales.setAdaptedYScale(graphSize.height, d[1] - d[0]);
  axis.drawYAxis(yScale, graphSize.height);
  legend.drawLegend(colors, g, graphSize.width);
  g.selectAll('g.bars').data(stackedData).enter().append('g').style('fill', function () {
    return colors[idx - 1];
  }).selectAll('rect').data(function (d) {
    return d;
  }).enter().append('rect').attr('x', function (d) {
    ...
    return yScale(d[0]) - yScale(d[1]);
  }).attr('width', 100).attr('class', 'rec').on('mouseover', tip.show).on('mouseout', tip.hide);
}
/**
...
function setClickHandler(g, graphSize) {
  d3.select('.button22').on('click', function () {
    viz2.build(g, graphSize);
  });
}
},{"./axis2.js":"scripts/viz2/axis2.js","./scales.js":"scripts/viz2/scales.js","./legend.js":"scripts/viz2/legend.js","./viz2.js":"scripts/viz2/viz2.js"}],"scripts/viz2/button.js":[function(require,module,exports) {
"use strict";
...
function drawButtons(g) {
  var button1 = g.append('g').attr('class', 'button22').attr('id', 'button22').attr('transform', 'translate(640, 140)').attr('width', 130).attr('height', 25);
  var buttons = d3.selectAll('.button22 ');
  buttons.append('rect').attr('width', 130).attr('height', 30).attr('fill', '#f4f6f4').on('mouseenter', function () {
    d3.select(this).attr('stroke', '#362023');
  }).on('mouseleave', function () {
    d3.select(this).attr('stroke', '#f4f6f4');
...
},{"d3-collection":"../node_modules/d3-collection/src/index.js","d3-selection":"../node_modules/d3-selection/src/index.js"}],"scripts/viz2/viz2.js":[function(require,module,exports) {
"use strict";
...
var preprocess = _interopRequireWildcard(require("./preprocess2.js"));
...
var bars = _interopRequireWildcard(require("./bars.js"));

var button = _interopRequireWildcard(require("./button.js"));
...
function build(g, graphSize) {
  g.selectAll('*').remove();
  helper.addFooter(g);
  g.append('text').text("Évolution de l'offre d'emploi par industrie").attr('x', graphSize.width / 2).attr('y', -75).attr('class', 'vizTitle');
  button.drawButtons(g);
  axis.appendAxes(g);
  axis.appendGraphLabels(g);
  var tip = (0, _index.default)().attr('class', 'd3-tip').html(function (d) {
    return tooltip.getContents(d);
  });
  d3.select('g').call(tip);
  d3.csv('./par_industrie_2019.csv').then(function (data2019) {
    d3.csv('./par_industrie_2021.csv').then(function (data2021) {
      var data1 = preprocess.filterRegion(data2019);
      var data2 = preprocess.filterRegion(data2021);
      var ind = preprocess.getIndustries(data1, data2);
      var xScale = scales.setXScale();
      var yScale = scales.setYScale(graphSize.height, ind);
      var colors = ['#0048BA', '#3B7A57', '#FDEE00', '#9F2B68', '#FBCEB1', '#89CFF0'];
      legend.drawLegend(colors, g, graphSize.width);
      axis.drawXAxis(xScale, graphSize.height);
      axis.drawYAxis(yScale, graphSize.height); // draw bars
...
exports.calculateDeficit = calculateDeficit;
exports.deficitScore = deficitScore;
exports.getCorrespondingPercentage = getCorrespondingPercentage;
...
function filterRegion(data) {
  data = data.filter(function (element) {
    return element.Nom_Region.indexOf('Ensemble') !== -1;
  });
  return data;
}
/**
...
function getSkills(data) {
  var skills = [];
  data.forEach(function (element) {
    var nbName = element.Genre_de_competences.trim();
    var nbCheck = skills.includes(nbName);
...
function structureData(data, skills) {
  var newData = [];
  var skillLevels = ['Haut. Qualifié', 'Qualifié', 'Moins Qualifié'];
  var skillDetection = {
    'Haut. Qualifié': 'Haut',
    Qualifié: '(Qual',
    'Moins Qualifié': 'Moins'
  };
  skills.forEach(function (skill) {
    var dataWithThisSkill = data.filter(function (element) {
      return element.Genre_de_competences.trim() === skill;
    });
    ...
    skillLevels.forEach(function (skillLevel) {
      var skillElement = {
        'Genre de compétences': skill
      };
      var dataWithThisSkillLevel = dataWithThisSkill.filter(function (element) {
        return element.Niveau_de_competences.indexOf(skillDetection[skillLevel]) !== -1;
      });
...
function calculatePercentage(dataWithOneSkill) {
  var percentages = {
    'Haut. Qualifié': 0,
    Qualifié: 0,
    'Moins Qualifié': 0
  };
  dataWithOneSkill.forEach(function (element) {
...
function calculateDeficit(dataWithOneSkill) {
  if (dataWithOneSkill.length === 0) {
    return 'Equilibre';
  }
  ...
  dataWithOneSkill.forEach(function (element) {
    if (element.Perspectives.indexOf('urplus') !== -1) {
      deficit += 1;
    }

    if (element.Perspectives.indexOf('ger D') !== -1) {
      deficit -= 1;
    }

    if (element.Perspectives === 'Deficit') {
      deficit -= 2;
    }

    if (element.Perspectives === 'Deficit important') {
      deficit -= 3;
    }

    if (element.Perspectives.indexOf('Non') !== -1) {
      countNonPublished += 1;
    }
  });
...
function getCorrespondingPercentage(skills, data) {
  var output = {};
  skills.forEach(function (skill) {
    data.forEach(function (element) {
      console.log(element);
...
},{}],"scripts/viz3/axis3.js":[function(require,module,exports) {
"use strict";
...
function appendGraphLabels(g, width, height) {
  var dy = -height / 2;
  var dx = width + 50;
  g.append('text').text("Pourcentage d'emplois par niveau de qualification").attr('class', 'axis-title').attr('font-size', 12).attr('transform', 'rotate(-90) translate(' + dy + ', -50)');
  g.append('text').text('Genre de').attr('class', 'axis-title').attr('font-size', 12).attr('transform', 'translate(' + dx + ',' + (height - 5) + ')');
  g.append('text').text('compétences').attr('class', 'axis-title').attr('font-size', 12).attr('transform', 'translate(' + dx + ',' + (height + 15) + ')');
}
/**
 * Draws the X axis at the bottom of the diagram.
 *
...
function rotateXTicks() {
  d3.select('.x-axis').selectAll('text').attr('class', 'axis-text').attr('transform', function () {
    var dy = -d3.select(this)._groups[0][0].__data__.length * 2.4;
    return 'rotate(-90) translate(' + dy + ',0)';
  }).style('font-weight', 'bold');
}
},{}],"scripts/viz3/scales.js":[function(require,module,exports) {
"use strict";
 ...
 */
function setRadiusScale(data2019, data2021) {
  var min2019 = d3.min(data2019, function (obj) {
    return Math.min(obj.emploi_estimé);
  });
  var max2019 = d3.max(data2019, function (obj) {
    return Math.max(obj.emploi_estimé);
  });
  var min2021 = d3.min(data2021, function (obj) {
    return Math.min(obj.emploi_estimé);
  });
  var max2021 = d3.max(data2021, function (obj) {
    return Math.max(obj.emploi_estimé);
  });
  var domain = [Math.min(min2019, min2021), Math.max(max2019, max2021)];
...
function setXScale(width, skills) {
...
},{}],"scripts/viz3/bubble.js":[function(require,module,exports) {
"use strict";
...
exports.drawBubbleEvolution = drawBubbleEvolution;

var preprocess = _interopRequireWildcard(require("./preprocess3.js"));
 ...
 */
function drawBubble(g, data, xScale, yScale, radiusScale, colorScale) {
  g.selectAll('circle').remove();
  g.selectAll('circle').data(data).enter().append('circle').attr('r', function (d) {
  ...
  }).style('stroke', '#D4D4D4').style('stroke-width', '1px');
}
/**
...
function drawBubbleEvolution(g, data2019, data2021, xScale, yScale, radiusScale, colorScale, skillLevel, skills) {
  data2019 = data2019.filter(function (element) {
    return element.niveau_de_competences === skillLevel;
  });
  data2021 = data2021.filter(function (element) {
    return element.niveau_de_competences === skillLevel;
  });
  g.selectAll('circle').remove();
  ...
  g.selectAll('.data19').data(data2019).enter().append('circle').attr('class', 'data19').style('stroke', '#000000').style('stroke-width', '2px').attr('r', function (d) {
    return Math.sqrt(radiusScale(d.emploi_estimé) / Math.PI);
  }).attr('cx', function (d) {
    return xScale(d['Genre de compétences']) + xScale.bandwidth() / 2;
  }).attr('cy', function (d) {
    ...
    return colorScale(d.niveau_de_competences);
  }).style('opacity', 0.7).transition().ease(d3.easeLinear).duration(1000).attr('cy', function (d) {
    return yScale(d.pourcentage);
  });
}
},{"./preprocess3.js":"scripts/viz3/preprocess3.js"}],"scripts/viz3/tooltip.js":[function(require,module,exports) {
"use strict";
...
var _default = index;
exports.default = _default;
},{"d3-selection":"../node_modules/d3-svg-legend/node_modules/d3-selection/index.js","d3-format":"../node_modules/d3-svg-legend/node_modules/d3-format/index.js","d3-dispatch":"../node_modules/d3-svg-legend/node_modules/d3-dispatch/index.js","d3-scale":"../node_modules/d3-svg-legend/node_modules/d3-scale/index.js","d3-array":"../node_modules/d3-svg-legend/node_modules/d3-array/index.js"}],"scripts/viz3/legend.js":[function(require,module,exports) {
"use strict";
...
exports.drawYearLegend = drawYearLegend;
...
function drawLegend(colorScale, g, width) {
  g.append('g').attr('class', 'legendOrdinal').attr('transform', 'translate(' + width + ',0)');

  var legendOrdinal = _d3SvgLegend.default.legendColor().shape('path', d3.symbol().type(d3.symbolCircle).size(200)()).shapePadding(10).cellFilter(function (d) {
    return d.label !== 'e';
  }).scale(colorScale).title('Légende');
...
function addComments(g, width) {
  g.append('text').text('Cliquez sur les éléments').attr('font-size', 12).attr('transform', 'translate(' + width + ',170)');
  g.append('text').text('de la légende pour gérer').attr('font-size', 12).attr('transform', 'translate(' + width + ',190)');
  g.append('text').text("l'affichage").attr('font-size', 12).attr('transform', 'translate(' + width + ',210)');
  g.append('text').text('E.D.S.C: Enseignement, droit,').attr('font-size', 12).attr('transform', 'translate(' + width + ',310)');
  g.append('text').text('serv. sociaux, communautaires').attr('font-size', 12).attr('transform', 'translate(' + width + ',330)');
  g.append('text').text('et gouvernement').attr('font-size', 12).attr('transform', 'translate(' + width + ',350)');
}
/**
...
function drawYearLegend(g, width) {
  var widthText = width + 25;
  g.selectAll('.legendYear').remove();
  g.append('circle').attr('fill', 'white').style('stroke', '#D4D4D4').style('stroke-width', '1px').attr('r', 10).attr('cx', width).attr('cy', 240);
  g.append('text').attr('class', 'legendYear').text('Données de 2021').attr('transform', 'translate(' + widthText + ', 245)');
  g.append('circle').attr('fill', 'white').style('stroke', '#000000').style('stroke-width', '2px').attr('r', 10).attr('cx', width).attr('cy', 270);
  g.append('text').attr('class', 'legendYear').text('Données de 2019').attr('transform', 'translate(' + widthText + ', 275)');
}
},{"d3-svg-legend":"../node_modules/d3-svg-legend/indexRollupNext.js"}],"scripts/viz3/viz3.js":[function(require,module,exports) {
"use strict";
...
exports.drawResetButton = drawResetButton;
exports.addLine = addLine;

var preprocess = _interopRequireWildcard(require("./preprocess3.js"));

var axis = _interopRequireWildcard(require("./axis3.js"));

var scales = _interopRequireWildcard(require("./scales.js"));

var bubble = _interopRequireWildcard(require("./bubble.js"));
...
function build(g, graphSize) {
  g.selectAll('*').remove();
  helper.addFooter(g);
  g.append('text').text('Distribution des emplois par genre de compétences').attr('x', graphSize.width / 2).attr('y', -75).attr('class', 'vizTitle');
  var width = graphSize.width - 220;
  var tip = (0, _index.default)().attr('class', 'd3-tip').html(function (d) {
    return tooltip.getContents(d);
  });
  g.call(tip);
  d3.csv('./par_profession_2019.csv').then(function (data2019) {
    d3.csv('./par_profession_2021.csv').then(function (data2021) {
...
function activateButton(g, data2019, data2021, xScale, yScale, radiusScale, colorScale, skills, tip, width) {
...
function drawResetButton(g, width, data, xScale, yScale, radiusScale, colorScale, tip) {
  var button = g.append('g').attr('class', 'button').attr('transform', 'translate(' + width + ', 100)').attr('width', 80).attr('height', 25);
...
function addLine(g, xScale, skills, height) {
  g.selectAll('rect').data(skills).enter().append('rect').attr('fill', 'grey').attr('x', function (d) {
    return xScale(d) + xScale.bandwidth() / 2;
  }).attr('width', 1).attr('height', height).style('opacity', 0.4);
}
},{"./preprocess3.js":"scripts/viz3/preprocess3.js","./axis3.js":"scripts/viz3/axis3.js","./scales.js":"scripts/viz3/scales.js","./bubble.js":"scripts/viz3/bubble.js","./tooltip.js":"scripts/viz3/tooltip.js","./legend.js":"scripts/viz3/legend.js","../helper.js":"scripts/helper.js","../../../node_modules/d3-tip/index.js":"../node_modules/d3-tip/index.js"}],"index.js":[function(require,module,exports) {
'use strict';

var helper = _interopRequireWildcard(require("./scripts/helper.js"));

var viz1 = _interopRequireWildcard(require("./scripts/viz1/viz1.js"));

var viz2 = _interopRequireWildcard(require("./scripts/viz2/viz2.js"));

var viz3 = _interopRequireWildcard(require("./scripts/viz3/viz3.js"));
...
},{"./scripts/helper.js":"scripts/helper.js","./scripts/viz1/viz1.js":"scripts/viz1/viz1.js","./scripts/viz2/viz2.js":"scripts/viz2/viz2.js","./scripts/viz3/viz3.js":"scripts/viz3/viz3.js"}],"../node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
